...
```{r}
ggplot(student_data,aes(x = absences, y = G3)) + geom_point(color = "blue") + geom_smooth()
```


```{r}
student_data_numeric = cbind(age, absences,G3)
cor(student_data_numeric)
```
...
```{r}
library(tidyverse)
ggplot(student_data,aes(x = age, y = G3)) + geom_point(color = "gold")  + geom_smooth()
```

```{r}
ggplot(student_data_no_zeroes,aes(x = age, y = G3)) + geom_point(color = "gold")  + geom_smooth()
```
...
```{r}
ggplot(student_data_no_zeroes,aes(x = absences, y = G3)) + geom_point(color = "blue")  + geom_smooth()
```
